...
extension WBControlView {
    @objc private func tapAction(sender _: UITapGestureRecognizer) {
        // 添加扭动动画
        fadeAnime()
        closeAnime { _ in
    ...
    @objc private func close(sender _: UIButton) {
        // 添加扭动动画
        fadeAnime()
        closeAnime { _ in
            self.removeFromSuperview()
    ...
    @objc private func buttonCancel(sender _: UIControl) {
